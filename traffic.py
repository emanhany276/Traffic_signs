...
epochs_val = 10     #how many iteration will go throw
...
images = []
classNo = []
myList = os.listdir(path) #return list of the names in the dir
...
############################### READ CSV FILE
data = pd.read_csv(labelFile) #use panda lib to read csv file
print("data shape ", data.shape, type(data)) #data.shape print (no.of rows,no.of col)
...
fig.tight_layout() # Adjusts the spacing between the subplots
for i in range(cols):
...
# is a technique used to artificially expand the size of a training dataset by creating modified versions of images in the dataset. This helps to prevent overfitting and improve the generalization of the model by providing it with more varied data
...
history = model.fit(
    ...
    shuffle=True  # Use True instead of 1 for better readability
)
############################### PLOT
plt.figure(1)
